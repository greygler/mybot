...
{
	const LINK_FB='https://graph.facebook.com/v2.9/';
	

	
private function users_pic($id,$users_profile_pic){
	$type_users_pic='.jpg';
	$users_pic_name=$id.$type_users_pic;
	$path = DIR_USERS_PIC.$users_pic_name;
	file_put_contents($path, file_get_contents($users_profile_pic));
	return $users_pic_name;
}

private function users_fb($id){
	...
	$users_pic_name=func::users_pic($id, $users['profile_pic']);
	$users['users_pic']=$users_pic_name;
	return $users;
 ...
 public function user_id_old($id) // Запрос данных о пользователе, возвращаем массив - СТАРАЯ!!!!
       {
		db::connect_db(DB_HOST,DB_NAME,DB_LOGIN,DB_PASS);
		$result = mysql_query("SELECT * FROM users WHERE user_id='{$id}'");
		$myrow = mysql_fetch_array($result);
		if (($myrow['id']!="") AND (($myrow['first_name']!="") OR ($myrow['last_name']!="")))  {
			...
			if ($users['first_name']!="") {
				...
				if (($id!="") AND ($id==BOT_ID)) $bd="INSERT INTO users (user_id, first_name) VALUES ('{$id}', '".BOT_NAME."')";  
				//if ($result == 'true') {echo "Информация добавлена успешно!";} else {echo "Информация не добавлена!";}
				else if ($id!=BOT_ID) $bd="INSERT INTO users (user_id) VALUES ('{$id}')";
...
public function user_id($id) // Запрос данных о пользователе, проверяем в базе, возвращаем массив
	{
		db::connect_db(DB_HOST,DB_NAME,DB_LOGIN,DB_PASS);
		$result = mysql_query("SELECT * FROM users WHERE user_id='{$id}'");
		if (mysql_num_rows($result) > 0 ) 
			{
				$myrow = mysql_fetch_array($result);
				if (($myrow['first_name']!="") OR ($myrow['last_name']!=""))  {
			
				$users=array(
					"first_name" =>  $myrow['first_name'],
					"last_name" => $myrow['last_name'],
					"profile_pic" => $myrow['profile_pic'],
					"users_pic" => $myrow['users_pic'],
					"locale" => $myrow['locale'],
					"timezone" => $myrow['timezone'],
					"gender" => $myrow['gender'],
					"is_payment_enabled" => $myrow['is_payment_enabled'],
					"users_group" => $myrow['users_group'],
					"db" => 'ok'
				);
			
				}
				else {
					$users=func::users_fb($id);
					$bd="UPDATE users SET first_name='{$users['first_name']}', last_name='{$users['last_name']}', profile_pic='{$users['profile_pic']}', users_pic='{$users['users_pic']}', locale='{$users['locale']}', gender='{$users['gender']}', is_payment_enabled='{$users['is_payment_enabled']}', users_group='1' WHERE user_id='{$id}'";
					$result = mysql_query($bd);
					
			
				}
		} else {
			if ($id==BOT_ID) $result = mysql_query("INSERT INTO users (user_id, first_name) VALUES ('{$id}', '".BOT_NAME."')");
			else{
			$users=func::users_fb($id);
			$bd="INSERT INTO users (user_id, first_name, last_name, profile_pic, users_pic, locale, timezone, gender, is_payment_enabled, users_group) VALUES ('{$id}', '{$users[first_name]}', '{$users[last_name]}', '{$users[profile_pic]}','{$users['users_pic']}', '{$users[locale]}','{$users[timezone]}','{$users[gender]}','{$users[is_payment_enabled]}','1')";
			$result = mysql_query($bd);
			$f = fopen("textfile1.json", "a");
                    fwrite($f, date("H:i:s")."\n");
					fwrite($f,"{$bd}\n");
					fclose($f);
			}
...
public function command_and_options($command) // Разделяем команду на команду и опцию, возвращаем массив
		{
...
public function smtpmail($to='', $mail_to, $subject, $message, $headers='', $config) { // Отправка письма через SMTP
...
public function func_replace($str, $user_id) // Замены в тексте сообщения
		{
...
public function last_message($user_id) // Находим последнее сообщение
		{
			db::connect_db(DB_HOST,DB_NAME,DB_LOGIN,DB_PASS);
...
public function curl_send($data_send_json, $mp) // Блок отправки - JSON данные, тип отправки
		{
...
// Работа с корзиной
public function products() { // Возвращаем товар в виде массива
		db::connect_db(DB_HOST,DB_NAME,DB_LOGIN,DB_PASS);
		$product=array();
...
public function view_cart($user_id){ // ID пользователя, возвращаем массив корзины
		db::connect_db(DB_HOST,DB_NAME,DB_LOGIN,DB_PASS);
		$result = mysql_query("SELECT * FROM users WHERE user_id='{$user_id}'");
		$myrow = mysql_fetch_array($result);
		$view_cart=unserialize($myrow['cart']);
		return $view_cart;
	}
	
	
public function add_to_cart($user_id, $cart) { // ID пользователя, массив с товарами, прописываем в корзину
		db::connect_db(DB_HOST,DB_NAME,DB_LOGIN,DB_PASS);
		$serialize_cart=serialize($cart);
		$db="UPDATE users SET cart='{$serialize_cart}' WHERE user_id='{$user_id}'";
		$result = mysql_query($db);
		return $result;
		
	}	
	
public function clear_cart($user_id) {  // ID пользователя, чистим корзину
		db::connect_db(DB_HOST,DB_NAME,DB_LOGIN,DB_PASS);
		$db="UPDATE users SET cart='' WHERE user_id='{$user_id}'";
		$result = mysql_query($db);
		return $result;
